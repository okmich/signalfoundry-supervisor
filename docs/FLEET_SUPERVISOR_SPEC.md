...
> **Account layout (2026-09-25).** `<live_base>` and `<log_base>` are laid out by account, `<base>\<account>\...`, the account being the broker env-file stem (`fxify.demo`). The operator chooses a system's account when importing it; the Supervisor groups the fleet by account folder, checks each running system's terminal login against the folder's env file, and refuses a start whose account env file is missing or incomplete (§7, §13, §16). Nothing is passed to a runner: it loads its own broker env, and the framework mirrors the account folder it is deployed in in its log paths. The Account Admin is an ordinary runner in its account folder whose governance is never undone as a side effect (§3, §16). See [ACCOUNT_LAYOUT_CHANGE_PLAN.md](../../signalfoundry-lab/docs/ops/ACCOUNT_LAYOUT_CHANGE_PLAN.md).
...
|---|---|---|---|
| **start-all** | `Stopped`, `StoppedByOperator` | live / in-flight (`Starting` / `Running` / `Stopping` / `Restarting`); **and** the fault states `Crashed` / `CrashLoopHalted` / `OrphanSuspected` | **Yes** |
| **stop-all** | `Running` | already-stopped; in-flight (`Stopping` / `Restarting`) | **Yes** |
| **restart-all** | `Running` | non-running (nothing to restart) | **Yes** |

- **start-all** starts what is stopped: `Stopped` (never run) and `StoppedByOperator` (stopped cleanly) — the cold-box case of [§13](#13-startup--operator-initiated-auto-start-deferred) ("open the Supervisor → start all"), including the whole box after maintenance or a reboot, when every system's last status is a clean stop. *(Revised 2026-09-26: start-all used to skip `StoppedByOperator`, which left it with nothing to start after any orderly stop.)* It does **not** override a fault — `Crashed` / `CrashLoopHalted` / `OrphanSuspected` ([§8](#8-lifecycle-state-machine)): clearing those is a **conscious per-system start**, never a side effect of a blanket "start everything." The confirm, which shows how many systems will start, is what stands between an operator and a fleet-wide start.
...
- **Nothing is passed to a runner.** The Supervisor starts `python -u run.py` (no arguments, cwd = its folder) with its own environment unchanged. The runner loads its broker env from its own `--env-file` default, as always; the framework derives the account from the folder the runner is deployed in and logs under `<log_base>\<account>\` (LOGGING_CONTRACT §10), so a supervised launch and a manual one behave the same. Choosing the right account at import is the operator's responsibility; the account check ([§7](#7-terminal--account-topology-and-blast-radius)) flags a runner whose terminal is logged into another account.
- **Failed starts explain themselves.** Each launch's console output (stdout and stderr) goes to `<log_base>\<account>\<runner root>\z_console_<UTC>.log`, beside the runner's own log. A runner that exits before reporting running fails the start at once, not at the start deadline: the system shows `Crashed` with a start error — the last line of that output — on its row ("start failed") and in its details, whose log pane shows the capture. It stays so until the system's next start.
- **Import** provisions an artefact into an account the operator picks — there is no default; the dialog refuses to validate until one is chosen (the TUI lists the accounts that have an env file): a staged copy, the current one archived to `.archive\<account>\...`, then an atomic rename. A runner artefact declares itself with `"runner": "<name>"` in its `config.json`. **Decommission** archives one system the Supervisor currently discovers — never a path built from an id — and refuses while it runs.
...
2. Define required machine-level env vars: `OKMICH_QUANT_LIVE_BASE`, `OKMICH_QUANT_LOG_BASE`, `OKMICH_QUANT_ENV_DIR`, and `OKMICH_QUANT_SUPERVISOR_STATE_DIR`.
...
> **Resolved 2026-09-25 — which account a system trades.** It was a hardcoded `--env-file` default inside each `run.py`, invisible to the Supervisor until the system ran. The operator now chooses it at import, which places the system in that account's folder ([§16](#16-folder-and-ipc-layout)); the fleet is grouped by it and each running system's terminal login is checked against it ([§7](#7-terminal--account-topology-and-blast-radius)). The runner itself is unchanged: it still loads its own env.
...
| **Account** | A broker env-file stem `<broker>.<env>` (`fxify.demo`): one env file, one terminal, one login. The first folder level under `<live_base>` (chosen at import) and, mirrored, under `<log_base>` ([§16](#16-folder-and-ipc-layout)). |
